...
public final class FindMeetingQueryTest {
  ...
  @Test
  public void busyOptionalAttendeeHasNoEffect() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    //
    // Events  :       |--A--|     |--B--|
    ...
    Collection<Event> events = Arrays.asList(
        ...
        new Event("Event 3", TimeRange.fromStartEnd(TimeRange.START_OF_DAY, 
            TimeRange.END_OF_DAY, true), Arrays.asList(PERSON_C))); // person c busy entire day
  ...
  public void optionalAttendeeIsConsidered() {
    ...
    Collection<Event> events = Arrays.asList(
        ...
        new Event("Event 3", TimeRange.fromStartEnd(TIME_0830AM, 
            TIME_0900AM, false), Arrays.asList(PERSON_C))); // person c busy for 30 min
  ...
  public void justEnoughRoomForOptionalAttendee() {
    ...
    MeetingRequest request = new MeetingRequest(Arrays.asList(PERSON_A), DURATION_15_MINUTES);
    request.addOptionalAttendee(PERSON_B);

    Collection<TimeRange> actual = query.query(events, request);
    Collection<TimeRange> expected =
        Arrays.asList(TimeRange.fromStartDuration(TIME_0845AM, DURATION_15_MINUTES));

    Assert.assertEquals(expected, actual);
  }

  @Test
  public void twoOptionalAttendees() {
  ...
  @Test
  public void noGapTwoOptionalAttendees() {
